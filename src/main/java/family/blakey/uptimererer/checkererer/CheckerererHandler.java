...
/**
 * Entry point when the checkererer runs as a Lambda behind the SQS event
 * source mapping. Each message body is a {@link Request}.
 *
 * A website that fails its ping is a successfully processed message — only
 * genuine processing failures (malformed body, DynamoDB unavailable) are
 * reported back to SQS, per message, for retry and eventual dead-lettering.
 */
